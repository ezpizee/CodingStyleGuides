...
`String unitAbbrev = "μs";`{.prettyprint .lang-java}

Best: perfectly clear even without a comment.

`String unitAbbrev = "\u03bcs"; // "μs"`{.prettyprint .lang-java}

Allowed, but there's no reason to do this.

`String unitAbbrev = "\u03bcs";       // Greek letter mu, "s"`{.prettyprint
.lang-java}

Allowed, but awkward and prone to mistakes.

`String unitAbbrev = "\u03bcs";`{.badcode}

Poor: the reader has no idea what this is.

`return '\ufeff' + content;        // byte order mark`{.prettyprint
.lang-java}
...
2.  When a line is broken at an *assignment* operator the break
    ...
    -   This also applies to the "assignment-operator-like" colon in an
        enhanced `for`{.prettyprint .lang-java} ("foreach") statement.
...
`XmlHttpRequest`{.prettyprint .lang-java}

`XMLHTTPRequest`{.badcode}

"new customer ID"

`newCustomerId`{.prettyprint .lang-java}

`newCustomerID`{.badcode}

"inner stopwatch"

`innerStopwatch`{.prettyprint .lang-java}

`innerStopWatch`{.badcode}

"supports IPv6 on iOS?"

`supportsIpv6OnIos`{.prettyprint .lang-java}

`supportsIPv6OnIOS`{.badcode}

"YouTube importer"

`YouTubeImporter`{.prettyprint .lang-java}\
 `YoutubeImporter`{.prettyprint .lang-java}\*
...
names `checkNonempty`{.prettyprint .lang-java} and
...
asterisk (`*`)—appears between paragraphs, and before the group of block
tags if present. Each paragraph but the first has `<p>` immediately
